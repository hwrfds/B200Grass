...
def lookup_tbl2_interp(df, baseline, w, ref_weight=12500, _debug=False, _st=None):
    ...
    if _debug and (_st is not None):
        _st.caption(f"[WeightAdj] weights={list(weights)} | w={w} → using {w1} & {w2} | alpha={alpha}")
        _st.caption(f"[WeightAdj] baseline={baseline:.2f} | d1={d1:.2f} | d2={d2:.2f} | delta={delta:.2f}")
...
def lookup_tbl4_interp(df, refd, _debug=False, _st=None):
    tbl       = df.sort_values(by=0).reset_index(drop=True)
    ref_rolls = tbl[0].values
    obs_vals  = tbl[50].values
    val = float(np.interp(refd, ref_rolls, obs_vals, left=obs_vals[0], right=obs_vals[-1]))
    if _debug and (_st is not None):
        _st.caption(f"[Obs50] x(min..max)=({ref_rolls[0]:.1f}..{ref_rolls[-1]:.1f}) | refd={refd:.2f}")
        _st.caption(f"[Obs50] y(min..max)=({obs_vals[0]:.1f}..{obs_vals[-1]:.1f}) | val={val:.2f}")
    return val

obs50 = lookup_tbl4_interp(df4, wind_adj, _debug=True, _st=st)
