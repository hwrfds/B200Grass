...
def lookup_tbl2_interp(df, baseline, w, ref_weight=12500):
    """
    Nearest-columns 2D interpolation for weight adjustment:
      1) Pick the closest lower and upper weight columns around w.
      2) For each, interpolate Δ vertically vs ref_weight at 'baseline'.
      3) Blend those two Δ by proximity to w.
      Returns: baseline + Δ at weight w.
    """
    import numpy as np
    import pandas as pd

    # Ensure numeric columns and sort by the reference column
    tbl = df.copy()
    ...
    # Reference x-axis (baseline distances at ref_weight)
    x_ref = tbl[ref_weight].values

    # Sorted available weight columns
    weights = np.array(sorted(int(c) for c in tbl.columns))

    # Find nearest lower/upper columns
    ...
    if idx == 0:
        ...
        lower = int(weights[idx-1])
        upper = int(weights[idx])
        if upper == w:
            w1 = w2 = upper
        else:
            w1, w2 = lower, upper
    ...
    # Step 1: vertical interpolation vs x_ref in each column
    ...
    # Step 2: horizontal blend by proximity in weight
    if w1 == w2:
        delta = d1
    else:
        alpha = (w - w1) / (w2 - w1)  # 0 at w1, 1 at w2
