...
def lookup_tbl2_interp(df, baseline, w, ref_weight=12500):
    """
    2-step linear interpolation over the weight-adjustment grid ("closest cells"):
      1) For each bounding weight column around `w`, interpolate Δ vertically vs the
         reference column (ref_weight) at the given `baseline`.
      2) Interpolate those two Δ values horizontally by weight to get Δ at `w`.
      3) Return baseline + Δ.
    """
    import numpy as np
    import pandas as pd

    # Ensure numeric columns and sort rows by the reference column
    tbl = df.copy()
    ...
    # Reference x-axis (baseline at ref_weight)
    x_ref = tbl[ref_weight].values

    # Available weight columns
    weights = np.array(sorted([int(c) for c in tbl.columns]))

    # Find bounding weights around requested w (clip to edges if needed)
    if w <= weights.min():
        w1 = int(weights.min())
        higher = weights[weights > w1]
        w2 = int(higher.min()) if higher.size else w1
    elif w >= weights.max():
        w2 = int(weights.max())
        lower = weights[weights < w2]
        w1 = int(lower.max()) if lower.size else w2
    else:
        w1 = int(weights[weights <= w].max())
        w2 = int(weights[weights >= w].min())

    # Compute column deltas relative to reference
    deltas_w1 = (tbl[w1].values - x_ref)
    deltas_w2 = (tbl[w2].values - x_ref)

    # Step 1: vertical interp in each column at the given baseline
    d1 = np.interp(baseline, x_ref, deltas_w1, left=deltas_w1[0], right=deltas_w1[-1])
    d2 = np.interp(baseline, x_ref, deltas_w2, left=deltas_w2[0], right=deltas_w2[-1])

    # Step 2: horizontal interp across weights
    if w1 == w2:
        delta = d1
    else:
        alpha = (w - w1) / (w2 - w1)
        delta = (1 - alpha) * d1 + alpha * d2
